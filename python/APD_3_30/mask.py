...
img_lena = np.array(img_lena)
[h_lena, w_lena, d] = img_lena.shape
img_3 = np.zeros((h, w, 3))

for y in range(h):
    for x in range(w):
        if y < h and x < w and y < h_lena and x < w_lena:
            if img_bin[y, x] == 0:
                img_3[y, x, :] = [0, 0, 0]
            else:
                img_3[y, x, :] = img_lena[y, x, :]
                
plt.figure()
plt.axis('off')
img_3 = img_3.clip(0, 255).astype(int)
